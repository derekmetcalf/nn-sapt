...
import os
import sys
from bayesian_dense import BayesianDense, VariationalRegularizer
...
NNunique = []
inputelement = []
sigma_regularization = 1e-3
mu_regularization = 1e-5
...
for i_type in range(ntype):
    ...
    layer = lambda output_dim, activation: BayesianDense(i_size_l, 
                                    activation='relu', 
                                    W_sigma_regularizer=VariationalRegularizer(
                                        weight=sigma_regularization),
                                    b_sigma_regularizer=VariationalRegularizer(
                                        weight=sigma_regularization),
                                    W_regularizer=regularizers.l1(mu_regularization))
    
    NN = layer(i_size_l, activation='relu')
    NNelement.append(NN)
    NN = layer(i_size_l, activation='relu')
    NNelement.append(NN)
    NN = layer(i_size_l, activation='relu')
    ...
    i_size_l = 1
    NN = layer(i_size_l, activation='linear')
    NNelement.append(NN)
...
for i_atom in range(len(aname)):
    ...
    NNatom=[]
    # loop over layers
    for i_layer in range(len(NNelement)):
