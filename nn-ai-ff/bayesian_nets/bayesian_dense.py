...
class BayesianDense(Layer):
    ...
    def build(self, input_shape):
        ...
        self.W_mu = self.init()#self.init((input_dim, self.output_dim),
    ...
    def call(self, x, mask=None):
        e = K.random_normal((int(x.shape[1]), self.input_dim, self.output_dim))
        #w = self.W_mu.dimshuffle('x',0,1)+e*(K.exp(self.W_log_sigma/2).dimshuffle('x',0,1))
        w = self.W_mu+e*(K.exp(self.W_log_sigma/2))
