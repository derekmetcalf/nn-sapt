...
def input_fn(inputdirs):
    for i in range(len(inputdirs)):
        ...
        for i in range(len(filenames)):
            ...
            sym_input.append(np.load(file).tolist())
...
def batched_sapt_routing(batch_sym,batch_atoms):
    batch_inps = (batch_sym, batch_atoms)
    batch_outs = tf.map_fn(sapt_net,batch_inps,dtype="float32")
    return batch_outs

def sapt_net(batch_inps):
    atom_inps = (batch_inps[0], batch_inps[1])
...
def build_np_dataset(sym_input,atoms_in,en_label,batch_size):
    batch_index = 0
    dataset = []
    sym_batch = []
    atom_batch = []
    ...
    for i in range(len(sym_input)):
        if batch_index < batch_size:
            sym_batch.append(sym_input[i])
            atom_batch.append(atoms_in[i])
            en_batch.append(en_label[i])
            batch_index += 1
        elif batch_index == batch_size:
            batch_index = 0
            dataset.append([sym_batch, atom_batch, en_batch])
            sym_batch = []
            atom_batch = []
            en_batch = []
        else:
...
if __name__ == "__main__":
    ...
    unique_atoms = tf.cast(unique_atoms, dtype=tf.string)
    symfuns = tf.placeholder(tf.float32, shape=[None, 26, 32])
    atoms = tf.placeholder(tf.string, shape=[None, 26])
    en_label = []
    for i in range(len(labels)):
        en_label.append(labels[i][0])

    dataset = build_np_dataset(sym_input,atoms_in,en_label,batch_size)
    #dataset = tf.data.Dataset.from_tensors((sym_input,atoms_in,en_label)).repeat().batch(batch_size)
    ...
    output = batched_sapt_routing(symfuns,atoms)
    with tf.Session() as sess:
        writer = tf.summary.FileWriter("summaries", sess.graph)

        label = tf.placeholder(tf.float32, shape=[None])
        loss = tf.losses.mean_squared_error(output/batch_size,label/batch_size)
        ...
        for epoch in range(training_epochs):
            ...
            for j in range(len(dataset)): 
                ...
                atom = dataset[j][1]
                y = dataset[j][2]
                _, c = sess.run([optimizer,loss],{symfuns: sym,
                                                atoms: atom,
                                                label: y})
                avg_cost += c / len(dataset)
            """
            for i in range(len(sym_input)):
        ...
        tf.saved_model.simple_save(sess, "./batch_model", inputs, outputs)
