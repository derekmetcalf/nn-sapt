...
(aname, xyz, elec, exch, ind, disp, energy, atom_tensor,
 en_units) = routines.read_sapt_data(inputdir)
t2 = time.time()
...
atom_tensor = np.reshape(
    atom_tensor, (np.shape(atom_tensor)[0] * np.shape(atom_tensor)[1]),
    order='F')
