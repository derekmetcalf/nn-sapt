...
for inputdir in inputdirs:
    ...
    data_obj = routines.DataSet("./crystallographic_data/2018-12-26-CSD-xyznma-meoh-159/FSAPT0/SAPT0-NRGS-COMPONENTS.txt",None,"kcal/mol")
    
    if data_obj.mode == "alex":
        print("alex mode engaged")
        (test_aname,test_atom_tensor,test_xyz,
            test_elec,test_ind,test_disp,test_exch,test_energy, test_geom_files,
            train_aname,train_atom_tensor,train_xyz,
            train_elec,train_ind,train_disp,train_exch,
            train_energy, train_geom_files) = data_obj.read_from_target_list()
        
    else:
        (aname, atom_tensor, xyz, elec, ind, disp, 
            exch,energy, geom_files) = data_obj.read_from_target_list()
    ...
    if data_obj.mode == "alex":
        ...
        train_atomic_num_tensor = routines.get_atomic_num_tensor(train_atom_tensor, atom_dict)
        compute_spatial_info(test_xyz, test_path)
        test_atomic_num_tensor = routines.get_atomic_num_tensor(test_atom_tensor, atom_dict)
        train_xyz = []
        test_xyz = []
    else:
        compute_spatial_info(xyz, path)
        atomic_num_tensor = routines.get_atomic_num_tensor(atom_tensor, atom_dict)
        xyz = []
    print("Constructing symmetry functions from distances, angles, atomic numbers, and hyperparameters...\n")
    t1 = time.time()
    if data_obj.mode == "alex":
        routines.construct_symmetry_input(NNff, train_path, train_atom_tensor, np.zeros(len(train_energy)), train_atomic_num_tensor, val_split=0)
        routines.construct_symmetry_input(NNff, test_path, test_atom_tensor, np.zeros(len(test_energy)), test_atomic_num_tensor, val_split=0)
    else:
        routines.construct_symmetry_input(NNff, path, atom_tensor, np.zeros(len(energy)), atomic_num_tensor, val_split=0)
    
    t2 = time.time()
    elapsed = (t2-t1)/60
    print("Done! %s minutes spent constructing symmetry functions.\n"%elapsed)
